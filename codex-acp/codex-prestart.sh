...
model="${MYCODEX_MODEL:-}"
# Exact model value previously generated by this recipe. When no current pin is
# requested, remove only that byte-exact line from persistent config.
legacy_model="${MYCODEX_LEGACY_MODEL:-}"

mkdir -p "${codex_home}"
touch "${config_file}"

awk -v model="${model}" -v legacy_model="${legacy_model}" '
  ...
  # Migrate the exact line generated by the old ChatGPT profile. Deliberately
  # preserve differently formatted lines and every other user-selected model.
  !in_table && model == "" && legacy_model != "" && \
    $0 == "model = \"" toml_escape(legacy_model) "\"" { next }
...
# This process is reached through the image entrypoint after Vaka has installed
# egress policy and permanently removed NET_ADMIN from the original process
# tree. The broker must remain in that tree: it, rather than docker exec, spawns
# each ACP adapter.
exec node /opt/codex-acp/acp-broker.mjs
